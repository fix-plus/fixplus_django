...
from fixplus.common.mixins import IsSuperAdminMixin, IsRegisteredMixin
from fixplus.common.pagination import LimitOffsetPagination, get_paginated_response_context
from fixplus.parametric.selectors.selectors import search_brand_name_list, get_timing_setting, search_device_type_list
from fixplus.parametric.serializers.serializers import (
...
class TimingSettingParametricApi(IsRegisteredMixin, APIView):
    ...
    def patch(self, request):
        ...
        if not instance:
            return Response({'detail': 'Timing setting not found.'}, status=status.HTTP_404_NOT_FOUND)

        serializer = InputTimingSettingParametricSerializer(instance, data=request.data,
                                                            partial=True)  # Use partial=True for PATCH
        serializer.is_valid(raise_exception=True)
