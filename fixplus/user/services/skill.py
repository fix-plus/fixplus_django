...
def create_technician_skill(
    ...
    try:
        ...
        if not technician.groups.filter(name='technician').exists(): raise Exception(_("Skills can only be assigned to technician users."))

        device_type_instance = DeviceTypeParametric.objects.get(title__iexact=device_type)
        ...
        if brand_names:
            brands = BrandNameParametric.objects.filter(title__in=brand_names)
        ...
        raise Exception(_("Error creating technician skill."))
...
def update_technician_skill(
    ...
    try:
        ...
        # Handle the case where the technician or device type does not exist
        raise Exception(_("Error update technician skill."))
