...
class EmailOrMobileBackend(ModelBackend):
    def authenticate(self, request, username=None, password=None, **kwargs):
        logger.debug(f"Attempting to authenticate account with username: {username}")
        ...
        try:
            ...
            logger.debug(f"Found account: {user}")
        except User.DoesNotExist:
            logger.debug("No account found with the provided username")
            return None
