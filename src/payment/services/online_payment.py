...
def initiate_customer_online_payment(
    ...
    try:
        ...
        sms_data = {
            ...
            'customer_name': f"{'آقای' if customer_payment.service.customer.gender == Customer.Gender.MALE else 'خانم'} {customer_payment.service.customer.full_name}",
            'amount': amount,
...
@transaction.atomic
def initiate_technician_online_payment(
        *,
        technician: User,
        amount: int,
        request,
    ...
    try:
        ...
        bank.set_amount(amount)
        bank.set_client_callback_url(reverse('callback-gateway'))
        bank.set_mobile_number(technician.mobile or '+989000000000')
        bank_record = bank.ready()

        technician_payment = TechnicianPayment.objects.create(
            technician=technician,
            online_amount=amount,
            online_bank=bank_record
        )
